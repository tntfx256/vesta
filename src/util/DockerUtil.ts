...
export class DockerUtil {
  ...
  public static installEngine() {
    if (!isRoot()) {
      return Log.error("You must run this command as root!");
    }
  ...
  public static installCompose() {
    ...
    ask<{ version: string }>({
      ...
      if (answer.version) {
        execute(
          `curl -L https://github.com/docker/compose/releases/download/${answer.version}/docker-compose-${getOutputOf("uname -s")}-${getOutputOf(
            "uname -m"
          )} > /tmp/docker-compose`
        );
